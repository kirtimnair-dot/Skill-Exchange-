...
import { 
  Search, Filter, Star, MapPin, Clock, Users, 
  Music, Palette, Code, Book, Heart, Utensils,
  Dumbbell, Globe, GraduationCap, Camera
} from 'lucide-react';
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-cyan-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">
                  {[...new Set(skills.map(skill => skill.userId))].length}
                </p>
              </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSkills.map((skill) => (
                ...
                >
                  {/* Category Badge */}
                  <div className={`p-4 bg-gradient-to-r ${getCategoryColor(skill.category)} text-white`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        {getCategoryIcon(skill.category)}
                        <span className="font-semibold">
                          {categories.find(c => c.id === skill.category)?.name || 'Skill'}
                        </span>
                      </div>
